...
const LPProRidersLisboa: React.FC = () => {
    ...
    React.useEffect(() => {
        fetch('/api/public/course-dates/lisboa-2026')
            .then(res => res.json())
            .then(data => {
                if (data?.course?.schedule && Array.isArray(data.course.schedule) && data.course.schedule.length > 0) {
                     setSchedule(data.course.schedule);
                }
            })
            .catch(err => console.error('Failed to fetch schedule from CRM:', err));
    }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                ...
                email: form.email,
                phone: form.phone,
                // Custom fields packed into notes or handled by backend extension
                interest: `PRORIDERS LISBOA 2026 - CPF: ${form.cpf}`,
                origin: 'LP_LISBOA',
                notes: `CPF: ${form.cpf}\nReason: ${form.reason}\nAssigned: Andre`
            };
            ...
            if (!res.ok) throw new Error('Failed to save to CRM');
            
            // WEBHOOKS (Optional: keep if needed for Make/Zapier)
                ...
                <div className="container mx-auto px-6 relative z-10">
                    ...
                    <div className="grid lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
                        {schedule.map((dayItem, index) => {
                            // Syles config based on index
                            const isGold = index === 2;
                            const isRed = index === 0;
                            
                            const labelBg = isGold 
                                ? "bg-zinc-800 border border-wtech-gold/30 text-wtech-gold" 
                                : isRed 
                                    ? "bg-wtech-red text-white" 
                                    : "bg-zinc-800 text-white";
                                    
                            const bulletColor = isGold 
                                ? "bg-wtech-gold" 
                                : "bg-wtech-red";

                            return (
                                <div key={index} className={`bg-zinc-900/50 border border-white/5 p-8 rounded-2xl transition-all hover:border-wtech-red`}>
                                    <div className="flex items-center justify-between mb-8">
                                        <span className="text-4xl font-black text-white/10">{String(index + 1).padStart(2, '0')}</span>
                                        <div className={`${labelBg} px-4 py-1 text-[10px] font-black uppercase tracking-widest italic`}>
                                            {dayItem.day}
                                        </div>
                                    </div>
                                    <h3 className="text-2xl font-black uppercase text-white mb-6 min-h-[64px]">{dayItem.title}</h3>
                                    <ul className="space-y-4">
                                        {dayItem.topics.map((topic: string, i: number) => (
                                            <li key={i} className="flex items-start gap-3 group">
                                                <div className={`w-1.5 h-1.5 rounded-full ${bulletColor} mt-2 shrink-0 group-hover:scale-150 transition-transform`}></div>
                                                <span className="text-gray-400 text-sm font-medium leading-tight">{topic}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
